 ...
 * @since 1.0
 */
class CUrlManager extends CApplicationComponent #rub
{
	const CACHE_KEY='Yii.CUrlManager.rules'; //cache  CUrlRule
	...
	protected function processRules()
	{
		...
		{
			$hash=md5(serialize($this->rules));//msa
	...
	public function parseUrl($request)
	{
		if($this->getUrlFormat()===self::PATH_FORMAT)
		{
			...
			//p($rawPathInfo); #string(15) "post/index.html" or string(0) ""
			$pathInfo=$this->removeUrlSuffix($rawPathInfo,$this->urlSuffix);
			#p($pathInfo);#string(10) "post/index"
			foreach($this->_rules as $i=>$rule)
			{
				if(is_array($rule))
					$this->_rules[$i]=$rule=Yii::createComponent($rule);
	...
	{
		#p($pathInfo); #string(11) "view/about/"
		if($pathInfo==='')
			return;
...
class CUrlRule extends CBaseUrlRule
{
	 ...
	 */
	 /*
	 *主要是为了生成左右两个正则，$pattern与$routePattern,$routePattern是基于$route与pattern的，$pattern是基于templete与pattern的
	 */
	public function __construct($route,$pattern)
	{
		if(is_array($route))#配置规则属性，第一个元素必为route
		{
		...
		$this->route=trim($route,'/');#初始化路由与模式
...
*/
		if(strpos($route,'<')!==false && preg_match_all('/<(\w+)>/',$route,$matches2))  //解析路由参数至引用
		{
			//p($matches2);
/*
array(2) {
  [0] => array(2) {
    [0] => string(12) "<controller>"
    [1] => string(8) "<action>"
  }
  [1] => array(2) {
...
/*
array(2) {
  ["controller"] => string(12) "<controller>"
  ["action"] => string(8) "<action>"
}
array(3) {
  ["module"] => string(8) "<module>"
  ["controller"] => string(12) "<controller>"
  ["action"] => string(8) "<action>"
}
*/
		...
		if(preg_match_all('/<(\w+):?(.*?)?>/',$pattern,$matches))#解析模式命名参数至tr
		{
			//p($matches);
/*
array(3) {
  [0] => array(3) {
    [0] => string(12) "<module:\w+>"
    [1] => string(16) "<controller:\w+>"
    [2] => string(12) "<action:\w+>"
  }
  [1] => array(3) {
    [0] => string(6) "module"
    [1] => string(10) "controller"
    [2] => string(6) "action"
  }
  [2] => array(3) {
    [0] => string(3) "\w+"
...
array(3) {
  ["/"] => string(2) "\/"
  ["<controller>"] => string(19) "(?P<controller>\w+)"
  ["<action>"] => string(15) "(?P<action>\w+)"
}
array(4) {
  ["/"] => string(2) "\/"
  ["<module>"] => string(15) "(?P<module>\w+)"
  ["<controller>"] => string(19) "(?P<controller>\w+)"
  ["<action>"] => string(15) "(?P<action>\w+)"
}
*/
		...
		#p($this->append);   #bool(true)
		$p=trim($p,'/');
		//p($p); #string(29) "<controller:\w+>/<action:\w+>"
		$this->template=preg_replace('/<(\w+):?.*?>/','<$1>',$p);
		//p($this->template);//string(30) "<module>/<controller>/<action>"
		...
		//p($this->pattern);
/*
string(42) "/^(?P<controller>\w+)\/(?P<action>\w+)\//u"
string(59) "/^(?P<module>\w+)\/(?P<controller>\w+)\/(?P<action>\w+)\//u"
*/
        //p($this->route);
/*
string(21) "<controller>/<action>"
string(30) "<module>/<controller>/<action>"
*/
		if($this->references!==array())
			$this->routePattern='/^'.strtr($this->route,$tr2).'$/u';
		//p($this->routePattern);
/*
string(41) "/^(?P<controller>\w+)\/(?P<action>\w+)$/u"
string(58) "/^(?P<module>\w+)\/(?P<controller>\w+)\/(?P<action>\w+)$/u"
*/
	 ...
	 */
	public function parseUrl($manager,$request,$pathInfo,$rawPathInfo) //得到请求参数，返回路由
	{
		//p(func_get_args());
		...
		$pathInfo.='/';
		#p($pathInfo); #string(11) "post/index/"
...
string(42) "/^(?P<controller>\w+)\/(?P<action>\w+)\//u"
*/
			#p($pathInfo); #string(21) "site/page/view/about/"
			#p($matches);
/*
array(5) {
  [0] => string(10) "site/page/"
...
*/
			...
			{
				#p($this->routePattern); #string(41) "/^(?P<controller>\w+)\/(?P<action>\w+)$/u"
				#p(strtr($this->route,$tr)); #string(10) "post/index"
				#p($this->route); #string(21) "<controller>/<action>"
